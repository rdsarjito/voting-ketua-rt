<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Tambah Kandidat</h2>
    </x-slot>
    ...
    <div class="bg-white dark:bg-gray-800 p-4 rounded shadow max-w-2xl">
        <form method="POST" action="{{ route('admin.candidates.store') }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea name="vision" class="w-full border rounded p-2 bg-gray-100 dark:bg-gray-700">{{ old('vision') }}</textarea>
                @error('vision')<div class="text-red-500 text-sm mt-1">{{ $message }}</div>@enderror
            </div>
            <div class="mb-4">
                <label class="block mb-1">Misi</label>
                <textarea name="mission" class="w-full border rounded p-2 bg-gray-100 dark:bg-gray-700">{{ old('mission') }}</textarea>
